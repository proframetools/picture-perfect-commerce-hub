...
CREATE POLICY "Order items admin access" ON public.order_items
FOR ALL
USING (is_super_admin())
WITH CHECK (is_super_admin());

-- product_reviews: Optimized policies
...
CREATE POLICY "Product reviews admin access" ON public.product_reviews
FOR DELETE
USING (is_super_admin())
WITH CHECK (is_super_admin());

-- uploaded_photos: Optimized policy
